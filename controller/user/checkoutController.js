  ...
  try {
    ...
    if (paymentMethod === 'razorpay') {
      if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
        // Payment failed - set flag for pending order creation
        req.paymentFailed = true;
        req.paymentFailureReason = 'Missing payment details';
        return next(); // Continue to checkout controller
      }
    ...
    // Set payment failed flag and continue
    req.paymentFailed = true;
...
const retryPayment = async (req, res) => {
  try {
    ...
    res.json({success: true,razorpayKey: process.env.RAZORPAY_KEY_ID,orderId: razorpayOrderId,
...
module.exports = { loadCheckout, checkout, orderSuccess, createRazorpayOrder, razorpayVerificationMiddleware, couponApply ,retryPayment,verifyRetryPayment }